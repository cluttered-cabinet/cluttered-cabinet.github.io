...
def make_sample_data(minimum, maximum, resolution=1):
    ...
    Returns:
        ...
        target (np.array): 10/0 output array based on rule (scaled for visualization)
    """
...
![[Implementing-Statistics_3_0.png]]
...
$$
f(\bar{X}) = \begin{cases} 1,  & x_{1} - x_{2} + \epsilon \geq -0.5 \\ 0, & \text{otherwise} \end{cases}
$$

Where $\bar{X} = (x_{1}, x_{2})$

So in the case of $\bar{X} = (2, 4)$ we have that $x_1 - x_2 = -2$. Adding the noise term $\epsilon$, if the result is greater than or equal to $-0.5$, our function outputs 1. That looks to check out when we look at our figure above.

Note: We're using 0 and 10 as our target values instead of the standard 0 and 1 - this makes the visualization clearer in the plots, and sklearn's LogisticRegression handles this automatically by treating any non-zero value as the positive class.
...
ax.set_zlabel(r"$f(x_{1},x_{2})$", rotation=0)
ax.view_init(15, 70)
...
![[Implementing-Statistics_8_0.png]]
...
![[Implementing-Statistics_14_0.png]]
