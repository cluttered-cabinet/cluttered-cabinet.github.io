...
2. **Precision**: This metric reflects the accuracy of the positive predictions your model makes.[^mn-precision-recall] In marketing terms, if you optimize for precision, your model will be more conservative, only assigning high propensity scores to those who are very likely to convert. This can mean fewer leads, but the leads you get are more likely to be of higher quality.

3. **Accuracy**: This is the proportion of true results among the total number of cases examined. For a marketing model, high accuracy means that the model is generally good at identifying both interested and not interested customers. However, it doesn't necessarily mean it's excellent at identifying the most valuable leads — just that it's good at not making mistakes.[^mn-imbalance]

4. **AUC - Area Under the Curve**: AUC refers to the area under the ROC curve,[^cite-roc] a graph that shows the performance of a classification model at all classification thresholds. AUC measures how well the model distinguishes between classes (e.g., buyers vs. non-buyers).[^mn-auc-prob] Optimizing for a higher AUC can lead to a model that's better at ranking leads by their likelihood to convert, rather than just classifying them.

5. **F1 Score**: The F1 score is the harmonic mean of precision and recall, and it's a balance between the two.[^mn-harmonic] In marketing, a high F1 score means the model does a good job of identifying leads without including too many false positives or missing too many true leads. It's particularly useful when you want a balance between finding as many leads as possible (recall) and ensuring those leads are actually likely to convert (precision).
...
[^mn-precision-recall]: Precision and recall pull in opposite directions: tightening the threshold raises precision but lowers recall, and vice versa. This is the precision–recall tradeoff.

[^mn-imbalance]: This is exactly why accuracy is misleading on imbalanced data — a model that always predicts "not interested" can score 95% accuracy while finding zero leads.

[^cite-roc]: The ROC (Receiver Operating Characteristic) curve originated in signal-detection theory during WWII radar analysis, plotting true-positive rate against false-positive rate across thresholds.

[^mn-auc-prob]: AUC has a clean probabilistic reading: it's the probability that a randomly chosen positive is ranked above a randomly chosen negative.

[^mn-harmonic]: The harmonic mean is used rather than the arithmetic mean because it punishes imbalance — F1 is only high when *both* precision and recall are high.
